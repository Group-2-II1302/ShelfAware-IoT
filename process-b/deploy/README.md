...
Use this when you're actively integrating, demoing, or debugging — not for production. The process lives inside your SSH session: you see logs in real time, you stop it with `Ctrl+C`, and it dies if you log out. For a real deploy that survives reboots and disconnects, see [Step-by-step deployment](#step-by-step-deployment) below (systemd).

### One-time setup (do this once per fresh Pi / clone)

```bash
ssh group2@<pi-host>            # tailscale: e.g. lightbox
cd ~                            # or wherever you keep the repo
git clone <repo-url> ShelfAware-IoT   # skip if already cloned
cd ShelfAware-IoT/process-b

python3 -m venv ~/process-b-venv      # create venv outside the repo so it survives git operations
source ~/process-b-venv/bin/activate
pip install -e .                      # install Process B + its deps

# create your env file once, edit values to match the backend / shelf provisioning:
cp deploy/process-b.env.example ~/process-b.env
chmod 600 ~/process-b.env
nano ~/process-b.env
```

Required env vars (see `process-b.env.example` for the full list and defaults):

- `PI_API_KEY` — bearer token for `/telemetry` and `/commands`.
- `BACKEND_URL` — e.g. `https://shelfaware-backend.<owner>.workers.dev`.
- `SHELF_IDS` — comma-separated UUIDs this Pi is responsible for.
- `DB_PATH` — absolute path to the SQLite outbox file (e.g. `/home/group2/ProcessB.sqlite`). Process B will create the file and schema on first run if it doesn't exist.
- `UDP_LISTEN_HOST` / `UDP_LISTEN_PORT` — where to receive datagrams from Process A.
- `PROC_A_CONTROL_HOST` / `PROC_A_CONTROL_PORT` — where to send `wake` commands.

### Every time you want to run it (e.g. after Pi reboot)

```bash
ssh group2@<pi-host>

cd ~/ShelfAware-IoT/process-b
git pull                              # pick up any new code
source ~/process-b-venv/bin/activate
set -a; source ~/process-b.env; set +a    # load env vars into this shell
process-b                             # foreground; Ctrl+C to stop
```

If `process-b` says "command not found" the venv isn't activated — re-run the `source` line. If config is missing, you'll get a clear `ConfigError` listing which env vars failed.
...
```bash
# outbox should hover near zero, max_rejects should be 0:
sqlite3 "$DB_PATH" "SELECT COUNT(*) AS pending, MAX(reject_count) AS max_rejects FROM pending_readings;"
```

In Process B's own logs you should see, every few seconds:

- `"telemetry POST accepted","accepted":N,"skipped":0,"deleted":N` — drainer is happy.
- `"HTTP Request: GET .../commands "HTTP/1.1 200 OK"` — poller is happy.

If you see `"telemetry POST permanent failure"` or growing backoff, check the env vars and that the backend is reachable.

### Stopping it

`Ctrl+C` in the terminal running `process-b`. Graceful shutdown takes a second or two as the listener and drainer finish in-flight work.

### Survives SSH disconnects?

No — the process is a child of your shell. If you want it to survive disconnects without going full systemd, use `tmux`:

```bash
tmux new -s shelfb            # start a persistent session
# (do the run steps above)
# Ctrl+B then D               # detach; Process B keeps running

tmux attach -t shelfb         # later: re-attach to see logs
```

`tmux` doesn't survive a Pi reboot. For that, use systemd.
